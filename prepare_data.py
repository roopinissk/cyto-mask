...
for i in tqdm(range(0,bf.shape[0])):
    # save channel 1 as a png file in ./dataset/bf
    bfi = bf[i]
    ...
    # plt.axis("off")
    # plt.savefig(f'./dataset/mask/{i}.png')
    # save channel 3 as a png file in ./dataset/mask
    io.imsave(f'./dataset/mask/{i}.png', maski)

# print(bf.shape)
# print(mask.shape)

mask_path = "dataset/mask/0.png"

# read mask as numpy array
mask = io.imread(mask_path)
print(mask.shape)

# I have an image with regions of interest in different colors.
# I want to convert these regions of interest into binary masks for each color.
# The image is a 2D numpy array where each pixel value is a color.
# I want to convert this image into a binary mask where each region of interest is a binary mask.
# I will iterate over each unique color in the image and create a binary mask for each color.
...
# Create a dictionary where the key is the color and the value is the binary mask
binary_masks = {}
for color in unique_colors:
    # Create a binary mask where the pixels with the color are 1 and the rest are 0
    binary_mask = np.where(mask == color, 1, 0)
    # Add the binary mask to the dictionary
    binary_masks[color] = binary_mask

# Save the binary masks as arrays
output_dir = './dataset/binary_masks'
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

for color, binary_mask in binary_masks.items()[1:]:
    print(f"Saving binary mask for color {color}")
    io.imsave(f'{output_dir}/{color}.png', binary_mask.astype(np.uint8) * 255)

# Plot the binary masks
fig, axes = plt.subplots(1, len(binary_masks), figsize=(20, 10))
